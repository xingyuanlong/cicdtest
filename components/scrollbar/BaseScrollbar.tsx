...
import useIsIos from '../_util/hooks/useIsIos'
import useReactivated from '../_util/hooks/useReactivated'
import useConfigInject from '../_util/hooks/useConfigInject'
import { getPreciseEventTarget } from '../_util/util'
...
export const Scrollbar = defineComponent({
  ...
  setup (props) {
    ...
    function syncScrollState (): void {
      ...
      if (container) {
        ...
        containerScrollLeftRef.value =
          container.scrollLeft * (direction.value === 'rtl' ? -1 : 1)
      }
    }
    ...
    function handleXScrollMouseDown (e: MouseEvent): void {
      ...
      memoMouseX = direction.value === 'rtl'
    ...
    function handleXScrollMouseMove (e: MouseEvent): void {
      ...
      const { value: container } = mergedContainerRef
      if (container) {
        container.scrollLeft = toScrollLeft * (direction.value === 'rtl' ? -1 : 1)
    ...
    watchEffect(() => {
      ...
      if (xRailEl) {
        if (!needXBar) {
          xRailEl.classList.add(`${prefixCls.value}-rail--disabled`)
        } else {
          xRailEl.classList.remove(
            `${prefixCls.value}-rail--disabled`
          )
        }
      }
      if (yRailEl) {
        if (!needYBar) {
          yRailEl.classList.add(`${prefixCls.value}-rail--disabled`)
        } else {
    ...
    return {
      ...exposedMethods,
      mergedClsPrefix: prefixCls.value,
      rtlEnabled: direction.value === 'rtl',
          ...
          {h(
            ...
            triggerIsNone ? null : { name: 'fade-in-transition' },
            {
              default: () =>
                this.needYBar && this.isShowYBar && !this.isIos ? (
                  <div
                    class={`${mergedClsPrefix}-rail__scrollbar`}
                    style={{
      ...
      return h(
        ...
        mergeProps(this.$attrs, {
          ...
          class: [
            `${mergedClsPrefix}`,
            this.themeClass,
            rtlEnabled && `${mergedClsPrefix}--rtl`
          ],
          style: this.cssVars,
        ...
        [
          this.container ? (
            ...
            >
              <ResizeObserver onResize={this.handleContentResize}>
                {{
                  default: () => (
                    <div
                      ref="contentRef"
