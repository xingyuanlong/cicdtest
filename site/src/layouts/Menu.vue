<template>
  <pf-scrollbar>
    ...
    >
      <pf-menu-item v-if="showOverview" key="/components/overview">
      ...
      <template v-for="m in menus">
        <template v-if="m.children">
          <pf-menu-item-group :key="m.order" :title="isZhCN ? m.title : m.enTitle">
            <template v-for="n in m.children">
              <pf-menu-item v-if="n.path" :key="n.path">
    ...
    watch(
      ...
      () => {
        ...
        let title = props.isZhCN
          ? 'pf-ui-vue - 一套企业级 Vue 组件库'
          : 'pf-ui-vue — An enterprise-class UI components based on Ant Design and Vue.js';
        if (item && item.title) {
          title = props.isZhCN
            ? `${item.subtitle || ''} ${item.title} - pf-ui-vue`
            : `${item.enTitle || item.title} - pf-ui-vue`;
        }
